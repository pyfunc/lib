...
current_version=$(grep -o "__version__ = version = '[0-9.]*'" src/pyfunc2/_version.py | grep -o "[0-9.]*")
...
cat > src/pyfunc2/_version.py << EOF
...
echo ""
git add src/pyfunc2/_version.py setup.py "$changelog_file" increment_init.py increment_setup.py
