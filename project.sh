...
current_version=$(grep -o "__version__ = version = '[0-9.]*'" automatyzer_desktop/_version.py | grep -o "[0-9.]*")
...
cat > automatyzer_desktop/_version.py << EOF
...
echo ""
git add automatyzer_desktop/_version.py setup.py "$changelog_file" increment_init.py increment_setup.py
...
rm -rf build/ dist/ automatyzer_desktop.egg-info/
